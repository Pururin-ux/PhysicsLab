...
For `apps/web` work, use `docs/current/START_HERE.md` as the entry point, then
read only the current documents and route/code relevant to the task.
Instruction-only work does not require reading application code unless it is
needed to resolve a specific documentation claim. Do not reconstruct the
...
`docs/current/QUALITY.md` records the same risk-based budget requested by Sasha
on 2026-09-12. Release checks and acceptance for specific critical flows still
