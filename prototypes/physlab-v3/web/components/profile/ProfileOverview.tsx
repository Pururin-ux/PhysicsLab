...
const readinessAccent: Record<Readiness["tone"], string> = {
  neutral: "border-l-white/25",
  cyan: "border-l-nova-cyan/70",
  gold: "border-l-nova-gold/70",
};
...
export function ProfileOverview() {
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Готовность — «шапка» экрана: задаёт тон, а не теряется в списке карточек. */}
      <section aria-label="Уровень готовности">
        <Card
          className={`flex flex-col gap-3 border-l-2 border-white/[.08] !p-5 md:!p-6 ${readinessAccent[overall.tone]}`}
        >
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="physics-number text-[26px] font-bold leading-none text-nova-gold">
                {xp}
              </span>
          ...
          <p className="max-w-[560px] text-[13px] leading-[1.65] text-white/60">
            <MathText
              text={`${overall.note} Оценка построена только на твоих ответах в тренажёре — **это не прогноз балла на ЦЭ/ЦТ**.`}
            />
          </p>
        </Card>
      </section>
      ...
      <section aria-label="План на сегодня">
        <Card
          className={`flex flex-col gap-4 border-l-2 !p-5 md:flex-row md:items-center md:justify-between md:!p-6 ${
            nextStep.tone === "gold"
