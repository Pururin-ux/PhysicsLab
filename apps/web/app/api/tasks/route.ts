...
function buildTopicMix(
  ...
  const learningOrder = orderTaskFamiliesByConceptGraph(templates);

  if (count !== 10) {
    return Array.from(
      { length: count },
      (_, slot) => learningOrder[(batch + slot) % learningOrder.length],
    );
  }

  // Сначала сохраняем калиброванный ритм 5/3/2, а внутри каждой сложности
  // выбираем семейства в порядке графа понятий. Так ученик встречает новые
  // навыки последовательно, но не теряет разнообразие вариантов.
