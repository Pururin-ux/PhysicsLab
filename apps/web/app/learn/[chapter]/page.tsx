...
export default async function TextbookChapterPage({ params }: Props) {
  ...
  return <article className="mx-auto flex w-full max-w-[800px] flex-col gap-8 text-[var(--text-primary)]">
    ...
    <Button asChild size="lg"><Link href={chapter.practice.href}>{chapter.practice.label}</Link></Button>
    {next && next.grade === chapter.grade && chapter.practice.href !== `/learn/${next.id}` && <Link href={`/learn/${next.id}`} className="inline-flex min-h-11 items-center text-[var(--action-primary)]">Следующее объяснение: {next.title} →</Link>}
