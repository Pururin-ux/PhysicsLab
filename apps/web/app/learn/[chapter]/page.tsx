...
export default async function TextbookChapterPage({ params }: Props) {
  ...
  return <article className="mx-auto flex w-full max-w-[800px] flex-col gap-8 text-[var(--text-primary)]">
    ...
    {!isSpeed && next && next.grade === chapter.grade && chapter.practice.href !== `/learn/${next.id}` && <Link href={`/learn/${next.id}`} className="inline-flex min-h-11 items-center text-[var(--action-primary)]">Следующее объяснение: {next.title} →</Link>}
