...
export const phaseChangeHeatBlueprint: TaskBlueprint = {
  ...
  textTemplate: (p) =>
    `${contextFor(p)}. Масса льда ${formatAnswerValue(p.m)} кг, начальная температура ${p.temp0} °C. Сколько теплоты нужно, чтобы нагреть лёд до 0 °C и полностью расплавить? Удельная теплоёмкость льда ${formatAnswerValue(ICE_SPECIFIC_HEAT_KJ)} кДж/(кг·°C), удельная теплота плавления λ = ${ICE_FUSION_HEAT_KJ} кДж/кг.`,
