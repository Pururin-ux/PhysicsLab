...
const accentStyles = {
  ...
  gold: {
    icon: "border-ambient-warm/40 bg-ambient-warm/[.1] text-ambient-warm",
    eyebrow: "text-ambient-warm",
    rule: "from-ambient-warm/75 via-ambient-warm/20 to-transparent",
  },
} as const;
...
export function StudyCompanion() {
  ...
  return (
    ...
    >
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -right-16 -top-24 h-64 w-64 rounded-full bg-ambient-warm/[.07] blur-3xl"
      />
      <div
        aria-hidden="true"
      ...
      <div className="relative grid min-h-[300px] md:grid-cols-[210px_minmax(0,1fr)] lg:grid-cols-[238px_minmax(0,1fr)]">
        <aside className="relative flex min-h-[146px] items-center gap-4 overflow-hidden border-b border-white/[.09] bg-space-950/35 px-5 py-5 md:min-h-full md:flex-col md:items-start md:justify-between md:border-b-0 md:border-r md:px-6 md:py-7">
          <div className="relative z-10 min-w-0">
            <p className="text-[10px] font-[800] uppercase tracking-[.18em] text-ambient-warm/85">
              Короткая подсказка
            </p>
                ...
                >
                  <span
                    className={`block rounded-full transition-[width,background-color,box-shadow] duration-200 ${
                      index === activeIndex
                        ? "h-2 w-6 bg-mode-learn shadow-[0_0_12px_rgba(6,186,213,.3)]"
