...
function SameForceExperiment({ run, reveal, compact = false }: { run: number; reveal: boolean; compact?: boolean }) {
  ...
  const cases = [
    { name: "Лёгкая каретка", mass: "1 кг", acceleration: "4 м/с²", speed: "4 м/с", path: "2 м" },
    { name: "Тяжёлая каретка", mass: "2 кг", acceleration: "2 м/с²", speed: "2 м/с", path: "1 м" },
  ];
  return <figure className={cn(styles.experiment, compact && styles.experimentCompact)} aria-labelledby="same-force-caption">
    <div className={styles.experimentHead}><div><p className={styles.eyebrow}>Обе стартуют из покоя</p><h3>Одна сила · один и тот же интервал</h3></div><span className={styles.timeChip}>4 Н · 1 с</span></div>
    <div className={styles.experimentRows} aria-live="polite">
      {cases.map((item,index)=><div className={styles.experimentRow} key={item.name}>
        <strong className={styles.caseName}>{item.name}</strong>
        <span><small>масса</small><b>{item.mass}</b></span>
        <span><small>сила</small><b>4 Н →</b></span>
        {reveal?<motion.span key={`${run}-${item.name}`} className={styles.caseResult} initial={reduceMotion?false:{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={reduceMotion?{duration:0}:{duration:0.35,delay:index*0.1}}><small>через 1 с</small><b className={styles.caseMeasures}><span><em>ускорение</em>{item.acceleration}</span><span><em>скорость</em>{item.speed}</span><span><em>путь</em>{item.path}</span></b></motion.span>:<span className={styles.casePending}><small>через 1 с</small><b>результат — ?</b></span>}
      </div>)}
    </div>
    <figcaption id="same-force-caption">{reveal ? "При той же силе лёгкая каретка получила вдвое большее ускорение, скорость и путь за первую секунду." : "Известны условия. Ускорение и результат появятся после запуска."}</figcaption>
  </figure>;
}

export function NewtonSecondLawTextbookExperiment() {
  const [run, setRun] = useState(0);
  const [reveal, setReveal] = useState(false);

  return <div className={styles.textbookExperiment}>
    <SameForceExperiment run={run} reveal={reveal} />
    <button type="button" className={styles.runButton} onClick={() => { setReveal(true); setRun((value) => value + 1); }}>
      {reveal ? "Сравнить ещё раз" : "Показать результат опыта"}
    </button>
  </div>;
}

function ForceRecord({ compact = false }: { compact?: boolean }) {
  return <figure className={cn(styles.forceFigure, compact && styles.forceFigureCompact)} aria-labelledby={compact?"worked-force-caption":"force-choice-caption"}>
    <div className={styles.forceRecord} role="img" aria-label="Сила тяги 6 ньютонов направлена вправо, сила сопротивления 2 ньютона — влево; результирующая сила равна 4 ньютонам вправо">
      <div><span>тяга</span><strong>6 Н <ArrowRight size={25} weight="bold" /></strong></div>
      <div><span>сопротивление</span><strong><ArrowLeft size={25} weight="bold" /> 2 Н</strong></div>
      <div className={styles.forceResult}><span>результат вдоль дорожки</span><strong>4 Н <ArrowRight size={29} weight="bold" /></strong></div>
      {compact?<p>По вертикали ускорения нет: <MathText text="$N=mg$" />.</p>:null}
    </div>
    <figcaption id={compact?"worked-force-caption":"force-choice-caption"}>{compact ? "Вертикальные силы уравновешены. Вдоль дорожки тяга и сопротивление направлены навстречу друг другу." : "Часть тяги уравновешивает сопротивление; оставшиеся 4 Н изменяют скорость."}</figcaption>
  </figure>;
}
...
function BrakingBusScene() {
  return <figure className={styles.brakingFigure} aria-labelledby="braking-scene-caption">
    <div className={styles.brakingArtWrap}>
      <Image className={styles.brakingArt} src={MIO_SCENES.acceleration} alt="Мио стоит в троллейбусе и держится за поручень во время движения" width={1536} height={1024} sizes="(max-width:760px) 100vw, 52vw" />
    </div>
    <div className={styles.brakingDirections} role="img" aria-label="Скорость троллейбуса направлена вправо, результирующая сила и ускорение — влево">
      <p><span>скорость</span><strong>вправо <ArrowRight size={25} weight="bold" /></strong></p>
      <p><span>результирующая сила</span><strong><ArrowLeft size={25} weight="bold" /> влево</strong></p>
      <p><span>ускорение</span><strong><ArrowLeft size={25} weight="bold" /> влево</strong></p>
    </div>
    <figcaption id="braking-scene-caption">Мио держится за поручень: при торможении скорость ещё направлена по ходу движения, а ускорение уже направлено обратно.</figcaption>
  </figure>;
}
...
export function DynamicsLesson() {
  ...
  const draftState = { step, prediction, observed, forceChoice, practiceForce, practiceAcceleration, practiceChecked, independentForce, independentAcceleration, independentChecked, brakingAnswer, brakingChecked, summaryText, summaryTried, summarySaved, investigationCompleted };
  ...
  function renderStep() {
    if (step === 0) return <div className={styles.openingStage}><figure className={styles.openingArt}><Image src={MIO_SCENES.inertia} alt="Мио придерживает лабораторную тележку с грузом и готовится сравнить её разгон" width={1536} height={1024} sizes="(max-width:760px) 100vw, 54vw" priority /><figcaption>Мио меняет массу каретки, но оставляет силу и время действия одинаковыми.</figcaption></figure><div className={styles.openingCopy}><p className={styles.eyebrow}>Перед опытом</p><h2 ref={headingRef} tabIndex={-1}>Почему лёгкую каретку разогнать проще, чем тяжёлую?</h2><p className={styles.lead}>Сила меняет скорость тела. Ускорение показывает, как быстро происходит это изменение. Сравним две лабораторные каретки: подействуем одинаковой силой в течение одной секунды, но массы будут разными.</p><div className={styles.startNotes} aria-label="Что понадобится в уроке"><p><span>сила</span> в обоих запусках одинакова</p><p><span>время</span> действия силы — одна секунда</p><p><span>масса</span> отличается в два раза</p></div></div></div>;
    ...
    if (step === 8) return <div className={styles.brakingStage}><div className={styles.brakingVisual}><BrakingBusScene /></div><div className={styles.brakingCopy}><p className={styles.eyebrow}>Теперь торможение</p><h2 ref={headingRef} tabIndex={-1}>Троллейбус едет вправо, а сила направлена влево</h2><p className={styles.lead}>Масса троллейбуса — 12 т, результирующая сила торможения — 36 кН влево. Найди модуль ускорения.</p><p className={styles.transferConversion}><MathText text="$12\,\text{т}=12\,000\,\text{кг}$" /> · <MathText text="$36\,\text{кН}=36\,000\,\text{Н}$" /></p><NumberField id="dynamics-braking" label="Модуль ускорения" value={brakingAnswer} onChange={(value) => { setBrakingAnswer(value); setBrakingChecked(false); }} suffix="м/с² влево" /><button type="button" className={styles.checkButton} onClick={() => setBrakingChecked(true)}>Проверить ответ</button>{brakingChecked ? <Feedback containerRef={feedbackRef} kind={brakingCorrect ? "good" : "try"}>{brakingCorrect ? "36 000 Н ÷ 12 000 кг = 3 м/с² влево. Ускорение направлено против скорости, поэтому троллейбус замедляется." : "Переведи тонны в килограммы и килоньютоны в ньютоны, затем раздели 36 000 Н на 12 000 кг."}</Feedback> : null}{brakingCorrect ? <div className={styles.practiceLinks} aria-label="Следующие задачи"><Link href="/practice/family/newton-second" className={styles.practiceLink}>Ещё задачи на второй закон <ArrowRight size={18} weight="bold" /></Link><Link href="/practice/family/friction-force" className={styles.practiceLinkSecondary}>Перейти к силе трения <ArrowRight size={18} weight="bold" /></Link></div> : null}</div></div>;

    return <div className={styles.summaryStage}><div className={styles.stageIntro}><p className={styles.eyebrow}>Итог</p><h2 ref={headingRef} tabIndex={-1}>Что осталось главным?</h2><p className={styles.lead}>Запиши связь своими словами — так будет проще узнать её в следующей задаче.</p></div><label className={styles.summaryLabel} htmlFor="dynamics-summary">Объяснение для себя</label><textarea id="dynamics-summary" className={styles.summaryInput} value={summaryText} onChange={(event) => { setSummaryText(event.target.value); setSummaryTried(false); setSummarySaved(false); setInvestigationCompleted(false); }} maxLength={10000} rows={5} placeholder="Например: при той же силе более тяжёлое тело получает меньшее ускорение…" aria-describedby="dynamics-summary-hint" /><p id="dynamics-summary-hint" className={styles.summaryHint}>Достаточно одной-двух фраз. Пиши так, как объяснил бы другу.</p><button type="button" className={styles.summaryButton} onClick={() => { if (!summaryReady) { setSummaryTried(true); setSummarySaved(false); setInvestigationCompleted(false); return; } const saved = lessonDraft.save({ ...draftState, summarySaved: true, investigationCompleted: true }); setSummarySaved(saved); setInvestigationCompleted(saved); }}>Сохранить итог</button>{summaryTried && !summaryReady ? <p className={styles.summaryError} role="alert">Добавь ещё немного слов — хотя бы одну законченную мысль.</p> : null}{summarySaved && !lessonDraft.error ? <div className={styles.practiceLinks} aria-label="Следующие задачи"><p>Итог сохранён в этом браузере. Можно потренироваться ещё.</p><Link href="/practice/family/newton-second" className={styles.practiceLink}>Ещё задачи на второй закон <ArrowRight size={18} weight="bold" /></Link></div> : null}</div>;
  }
  ...
  return (
    <LessonStageEngine
      ...
      identity={
        <div>
          <Link href="/learn" className={styles.topicBack}>
            <ArrowLeft size={14} weight="bold" /> К учебнику
          </Link>
