...
test("карточка типа ведёт в focused drill, неизвестный slug — 404", async ({ page }, testInfo) => {
  ...
  if (testInfo.project.name.startsWith("mobile")) {
    await expect(page.getByLabel("Прогресс задач: 1 из 5")).toBeVisible();
  } else {
    await expect(page.getByTitle("Прогресс задач: 0 из 5")).toHaveCount(1);
  }
  await expect(page.getByText("1 / 5", { exact: true }).filter({ visible: true }).first()).toBeVisible();
...
test("focused drill восстанавливает 3/5 и restart запрашивает новый batch", async ({ page, request }) => {
  ...
  await page.getByTestId("next-task-button").click();
  await expect(page.getByText("3 / 5", { exact: true }).filter({ visible: true }).first()).toBeVisible();
  ...
  for (let index = 3; index < 5; index += 1) {
    ...
    if (index === 4) {
      await expect(page.getByText("5 / 5", { exact: true }).filter({ visible: true }).first()).toBeVisible();
    }
    await page.getByTestId("next-task-button").click();
  }
