...
## Текущая runtime-библиотека

Единый реестр допустимых runtime-путей находится в
`lib/learning/mio-assets.ts`. Он не означает, что целевая библиотека из
`docs/current/MIO_CHARACTER.md` завершена.

| ID | Файл | Внутреннее действие | Ограничение |
| --- | --- | --- | --- |
| `thinking` | `mio-thinking-v1.png` | Формулирует вопрос, обдумывает следующий шаг | Не использовать как универсальную улыбку |
| `attentive` | `mio-attentive-v1.png` | Сверяет условие, наблюдение или результат | Не означает недовольство учеником |
| `skeptical` | `mio-skeptical-v2.png` | Проверяет слишком удобную гипотезу, включая собственную | Не использовать как наказание за ошибку |
| `surprised` | `mio-surprised-v1.png` | Реагирует на действительно неожиданное наблюдение | Не подменяет обычную подсказку или успех |
| `celebrate` | `mio-celebrate-v1.png` | Отмечает уже подтверждённый результат | Использовать редко, после действия ученика |

Сейчас одобрены пять портретных состояний. Они не образуют полноценный
expression sheet: не хватает как минимум спокойного нейтрального состояния,
замешательства, усталости, объяснения, слушания и сдержанной гордости. Pose
sheet также не готов. Контекстные `textbook-*` сцены считаются отдельными
действиями, а не вариантами одной портретной эмоции.

`mio-skeptical-v1.png` — rejected intermediate с остатками нарисованной
шахматной подложки в волосах. Файл сохраняется как производственная история,
но отсутствует в runtime-реестре и не должен подключаться к интерфейсу.

## Происхождение и проверки ассетов

`mio-thinking-v1.png`: прозрачный фон, поза размышления с блокнотом. Сейчас
используется на главной как приглашение сформулировать вопрос.
